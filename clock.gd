...
@export_group("Fixed or OffsetStart Time")
...
# some numeric precision issues but Godot uses 64-bit numbers
# so it will take such a long time to become an issue that we ignore it
